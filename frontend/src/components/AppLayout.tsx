...
export default function AppLayout({
  ...
  const { activeJob } = useImportProgress();
  const [collapsed, setCollapsed] = React.useState(false);
  const [mobileOpen, setMobileOpen] = React.useState(false);

  // Add bottom padding to main content when progress bar is visible
  const hasProgressBar = !!activeJob;
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-sky-page text-ink select-none">
      ...
      <div className="flex flex-1 flex-col min-w-0 h-screen overflow-hidden bg-sky-page">
        ...
        />
        <main className={`flex-1 min-h-0 overflow-y-auto p-6 animate-fadeIn motion-reduce:animate-none${hasProgressBar ? " pb-20" : ""}`}>
          {children}
        </main>
      </div>

      {/* Persistent import progress bar */}
      <ImportProgressBar />
    </div>
  );
}
