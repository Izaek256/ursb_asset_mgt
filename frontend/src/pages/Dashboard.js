...
const getStatConfig = (label) => {
    ...
    if (l.includes("request")) {
        return { icon: ICONS.requests, bg: "bg-stat-amberChip", text: "text-stat-amberIcon" };
    }
    if (l.includes("department")) {
        return { icon: ICONS.building, bg: "bg-stat-blueChip", text: "text-stat-blueIcon" };
    }
    if (l.includes("user")) {
        return { icon: ICONS.users, bg: "bg-stat-greenChip", text: "text-stat-greenIcon" };
    }
    return { icon: ICONS.assets, bg: "bg-stat-blueChip", text: "text-stat-blueIcon" };
};
export default function Dashboard({ onNavigate }) {
    ...
    return (_jsxs("div", { className: "w-full flex flex-col gap-6 select-none font-sans", children: [data.maintenance_due > 0 && (_jsxs("div", { className: "bg-badge-amberBg border border-badge-amberText/16 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("span", { className: "w-10 h-10 rounded-xl bg-white flex items-center justify-center text-badge-amberText shadow-sm shrink-0", children: _jsx(ICONS.maintenance, { className: "w-5 h-5 stroke-[2.2]" }) }), _jsxs("div", { className: "flex flex-col", children: [_jsx("span", { className: "font-bold text-ink text-sm", children: "Maintenance Schedule Warning" }), _jsxs("span", { className: "text-xs text-ink-dim mt-0.5", children: ["There are ", data.maintenance_due, " assets requiring scheduled maintenance or repairs."] })] })] }), _jsx(Button, { variant: "outline", onClick: () => onNavigate("/maintenance"), children: "View Schedule" })] })), _jsx("div", { className: "grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-4 sm:gap-5", children: statsList.map((s, index) => {
                    ...
                    return (_jsxs("div", { className: `bg-white border border-sky-cardBorder rounded-2xl p-5 flex items-center gap-3.5 shadow-sm hover:shadow-xl hover:shadow-ursb/20 hover:-translate-y-1.5 hover:border-transparent duration-300 transition-all opacity-0 animate-statIn motion-reduce:animate-none motion-reduce:opacity-100 ${delayClass}`, children: [_jsx("span", { className: `w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${cfg.bg} ${cfg.text}`, children: _jsx(StatIcon, { className: "w-5 h-5 stroke-[2.4]" }) }), _jsxs("div", { className: "min-w-0", children: [_jsx("span", { className: "block text-[10px] font-bold uppercase tracking-wider text-ink-dim truncate", children: s.label }), _jsx("span", { className: "block text-xl font-bold text-ink mt-0.5 font-sans leading-none", children: s.value !== undefined ? s.value.toLocaleString() : "—" })] })] }, s.label));
