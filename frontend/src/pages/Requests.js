...
export default function Requests() {
    ...
    const renderActions = (r) => (_jsxs("div", { className: "flex flex-wrap gap-1.5", children: [user?.role === "Employee" && r.status === "Pending" && (_jsx(Button, { variant: "danger-outline", onClick: () => handleCancelClick(r), children: "Cancel" })), user?.role === "Employee" && r.status === "Assigned" && (_jsx(Button, { variant: "outline", onClick: () => handlePickupClick(r), children: "Confirm Pickup" })), isAdminOrManager && r.status === "Pending" && (_jsxs(_Fragment, { children: [_jsx(Button, { onClick: () => handleApproveClick(r), children: "Approve" }), _jsx(Button, { variant: "danger-outline", onClick: () => handleRejectClick(r), children: "Reject" })] })), isAdminOrManager && r.status === "Approved" && (_jsx(Button, { variant: "outline", onClick: () => handleAssignClick(r), children: "Assign" })), isAdminOrManager && r.status === "PickedUp" && (_jsx(Button, { onClick: () => setCompleteConfirm(r), children: "Complete" }))] }));
    const columns = [
        {
            header: "Request ID",
        ...
        {
            header: "Asset / Type",
            render: (r) => r.asset_id ? (_jsxs("div", { children: [_jsx("div", { className: "font-bold text-ink text-sm", children: r.asset_name || "Asset" }), _jsx("div", { className: "text-[11px] text-ink-dim mt-0.5", children: r.asset_id })] })) : (_jsx("span", { className: "text-ink-dim", children: r.asset_type })),
        },
        {
            header: "Requested By",
            render: (r) => r.requested_by_name || `User ID: ${r.requested_by}`,
        },
        {
            header: "Priority",
