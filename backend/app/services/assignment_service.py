...
def initiate_return(db: Session, assignment_id: int, current_user_id: int) -> Assignment:
    """
    Step 1 of 2: Employee initiates asset return.

    Parameters:
        db: Database session.
        assignment_id: ID of the assignment to return.
        current_user_id: user_id of the employee initiating the return.

    Returns:
        Updated Assignment ORM object.

    Raises:
        HTTPException 404 if assignment not found.
        HTTPException 403 if current user is not the assigned_to user.
        HTTPException 422 if assignment is not in Active status.

    Business rules:
        An employee may only return their own assets — ownership enforced here.
        Assignment must be Active before return can be initiated.

    Status transitions:
        Asset status: Active → Returned
        Assignment status: Active → Return Initiated

    Audit log:
        Writes action=RETURN_INITIATED on success.

    Transaction boundary:
        Asset status update and assignment status update are atomic.
    ...
    if int(assignment.assigned_to) != current_user_id:
        raise HTTPException(status_code=403, detail="You can only return your own assets")

    if assignment.status != AssignmentStatus.ACTIVE:
        raise HTTPException(
            status_code=422,
            detail=f"Assignment must be Active to initiate return. Current status: {assignment.status.value if hasattr(assignment.status, 'value') else assignment.status}"
        )
    ...
    assignment.status = AssignmentStatus.RETURN_INITIATED
    ...
    db.add(AuditLog(
        ...
        details=f"Employee initiated return for assignment {assignment_id}",
    ))
    db.commit()
...
def confirm_return(db: Session, assignment_id: int, custodian_id: int) -> Assignment:
    """
    Step 2 of 2: Custodian confirms physical receipt of returned asset.

    Parameters:
        db: Database session.
        assignment_id: ID of the assignment being confirmed.
        custodian_id: user_id of the Custodian confirming the return.
    ...
    Raises:
        HTTPException 404 if assignment not found.
        HTTPException 422 if asset status is not Returned.

    Business rules:
        Any Custodian can confirm a return — no ownership restriction.
        Asset must be in Returned status before confirmation is allowed.

    Status transitions:
        Asset status: Returned → Available
        Assignment status: Return Initiated → Returned

    Audit log:
        Writes action=RETURN_CONFIRMED on success.

    Transaction boundary:
        All four database changes are atomic:
        asset status, assignment status, return_date, current_custodian_id.
    ...
    asset = db.query(Asset).filter(Asset.asset_id == assignment.asset_id).first()
    if not asset or asset.status != AssetStatus.RETURNED:
        raise HTTPException(
            status_code=422,
            detail=f"Asset must be in Returned status to confirm return. Current status: {asset.status.value if asset and hasattr(asset.status, 'value') else 'Unknown'}"
        )

    asset.status = AssetStatus.AVAILABLE
    # Clearing current_custodian_id is intentional — asset is no longer with any specific user
    ...
    db.add(AuditLog(
        ...
        table_affected="assignments",
        record_id=str(assignment_id),
        details=f"Custodian confirmed return for assignment {assignment_id}",
    ))
    db.commit()
    db.refresh(assignment)
    return assignment